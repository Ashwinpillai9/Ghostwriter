...
- **`keyboard.key_to_scan_codes("right ctrl")` includes Left Ctrl's own scan code (29).** Not
  an event-swallowing problem like Right Alt's AltGr — just a stale entry in the library's own
  Windows name table. Binding the name directly would suppress Left Ctrl too and let it fire
  the hotkey; `hotkeys._right_ctrl_codes()` filters 29 out before binding.
